...
#  app_manifest_version_code - set the apps version number.
{
  'variables': {
    ...
    'aar_excluded_classes': [
      '*/R.class',
      '*/R##*.class',
      '*/Manifest.class',
      '*/Manifest##*.class'
    ],
    'aar_jar_name': 'chromium_aar_<(_target_name).jar',
    'aar_name': '<(_target_name).aar',
    'aar_jar_path': '<(PRODUCT_DIR)/lib.java/<(aar_jar_name)',
              ...
              'actions': [
                {
                  ...
                  'includes': [ '../../build/android/create_standalone_apk_action.gypi' ],
                },
              ],
            }],
          ],
    ...
    ['create_abi_split == 1', {
      'actions': [
        ...
        },
        {
          ...
          'includes': [ '../../build/android/package_resources_action.gypi' ],
        },
        {
          'variables': {
            'apk_name': '<(main_apk_name)-abi-<(android_app_abi)',
          ...
          'includes': ['../../build/android/apkbuilder_action.gypi'],
        },
      ],
    }],
      ...
      'actions': [
        {
          ...
          'includes': [ '../../build/android/finalize_splits_action.gypi']
        },
      ],
    }],
    ['is_test_apk == 1', {
  ...
  'actions': [
    ...
    },
    {
      ...
      'includes': [ '../../build/android/instr_action.gypi' ],
    },
    {
      'variables': {
        'src_dirs': [
      ...
      'includes': [ '../../build/android/lint_action.gypi' ],
    },
    {
    ...
    {
      'action_name': 'aar_jar_<(_target_name)',
      'message': 'Creating <(_target_name) aar jar',
      'inputs': [
        '<(DEPTH)/build/android/gyp/util/build_utils.py',
      ...
      'action': [
        ...
        '--classes-dir=<(classes_dir)',
        '--jar-path=<(aar_jar_path)',
        '--excluded-classes=<(aar_excluded_classes)',
        '--stamp=<(aar_jar_stamp)',
      ]
    },
    {
      'action_name': 'generate_aar_<(_target_name)',
      'message': 'Generating <(_target_name) aar',
      'variables': {
        # Write the inputs list to a file, so that its mtime is updated when
        # the list of inputs changes.
        'inputs_list_file': '>|(generate_aar.<(_target_name).gypcmd >@(package_input_paths))'
      },
      'inputs': [
        '<(DEPTH)/build/android/gyp/util/build_utils.py',
        '<(DEPTH)/sippet/build/android/gyp/generate_aar.py',
        '<(aar_jar_stamp)',
        '<(codegen_stamp)',
        '<(obfuscate_stamp)',
        '<(unsigned_apk_path)',
      ...
      'action': [
        'python', '<(DEPTH)/sippet/build/android/gyp/generate_aar.py',
        '--native-libs-dir=<(apk_package_native_libs_dir)',
        '--android-manifest-path=<(android_manifest_path)',
        '--asset-dir=<(asset_location)',
        '--out-dir=<(intermediate_dir)',
        '--jar-path=<(aar_jar_path)',
        '--resource-dir=<(resource_dir)',
        '--aar-path=<(aar_path)',
        '--additional-resource-arr-paths=>(additional_resource_arr_paths)',
        '--input-jars-paths=>(input_jars_paths)',
      ]
    },
  ],
}
